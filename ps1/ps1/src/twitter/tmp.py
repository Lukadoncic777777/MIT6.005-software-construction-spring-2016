...
def pollTweets():
    ...
    FILENAME = "tweets-inclass.json"
    URL = 'https://api.twitter.com/1.1/statuses/user_timeline.json'
    USERS = ["neiltyson", "marscuriosity"]
    PARAMS = { "lang":"en", "exclude_replies":"true", "count":"100" }
    NUM_PAGES = 10
    ...
    if shouldPoll(FILENAME):
        ...
        writeTweetsOut(FILENAME, sortByDate(removeRepeats(statuses)))
...
def twitterSearch(numPages, url, params, keys):
    ...
    params = params.copy() # since we mutate params below!
...
def sortByDate(tweetList):
    return sorted(tweetList, key=lambda tweet: parse(tweet["created_at"]), reverse=True)
